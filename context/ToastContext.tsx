...
export const ToastProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addToast = useCallback((message: string, type: ToastType = 'info') => {
    ...
    // Aumentei o tempo para 5 segundos para garantir leitura
    setTimeout(() => {
  ...
  return (
    <ToastContext.Provider value={{ addToast }}>
      {children}
      {/* Container Centralizado com Backdrop */}
      <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center pointer-events-none p-4">
        
        {/* Backdrop Escuro (só aparece se houver toasts) */}
        {toasts.length > 0 && (
            <div className="absolute inset-0 bg-black/50 backdrop-blur-sm transition-opacity animate-in fade-in duration-200 pointer-events-auto" />
        )}
        ...
        <div className="z-10 flex flex-col gap-4 items-center w-full max-w-md pointer-events-none">
            ...
            <div
                ...
                toast.type === 'success' ? 'border-green-500 ring-4 ring-green-500/10' : 
                toast.type === 'error' ? 'border-red-500 ring-4 ring-red-500/10' : 'border-blue-500 ring-4 ring-blue-500/10'
                }`}
                role="alert"
            >
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-base font-medium text-gray-600 leading-snug">{toast.message}</p>
                </div>
                <button
